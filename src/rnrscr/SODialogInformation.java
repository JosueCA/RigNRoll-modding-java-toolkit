...
implements IMissionInformation {
    ...
    public boolean checkTimePeriods(CoreTime time) {
    	// 
    	// RICK: added a Iterator to each MissionDialogs object
    	// 
        Iterator<MissionDialogs.dayTimePeriod> daytime_iter;
        Iterator<MissionDialogs.TimePeriod> timeperiods_iter;
        ...
        if (0 != this.timeperiods.size()) {
            timecheck = false;
            timeperiods_iter = this.timeperiods.iterator();
            while (timeperiods_iter.hasNext()) {
                if (!((MissionDialogs.TimePeriod)timeperiods_iter.next()).isInPeriod(time)) continue;
                timecheck = true;
                break;
            }
        }
