...
public class LoadGameTable implements ISetupLine, ISelectLineListener {
  ...
  static class table_data {
    Vector<LoadGameTable.line_data> all_lines = new Vector<LoadGameTable.line_data>();
  }
  ...
  class EditName {
    ...
    void selectLine(int line) {
      ...
      if (this.lastline == line) {
        ...
        LoadGameTable.line_data data = (LoadGameTable.line_data)(LoadGameTable.this.table.getItemOnLine(line)).item;
        if (null != data.cb && 
          data.cb.clearOnEnterEdit(data.game_media))
    ...
    void dissmissSaveGameName(long _menu, MENUEditBox obj) {
      LoadGameTable.line_data data = (LoadGameTable.line_data)(LoadGameTable.this.table.getItemOnLine(this.lastline)).item;
      if (null != data.cb) {
  ...
  private String SAVETIME_ORDER_text;
  
  private String SAVETIME_ORDER_QR_text;
  ...
  private void buildvoidcells() {
    if (this.TABLE_DATA.all_lines.size() < this.table.getNumRows()) {
      ...
      while (iter.hasNext() && 
        ((line_data)iter.next()).wheather_show)
        count_good_data++; 
  ...
  public LoadGameTable(IWindowContext context, long _menu, String[] action_buttons, String table_name, String ranger_name, String xml_name, String lines_name, String[] lineselements, int type_game, int type_menu_load, boolean bIsClips, String sortByNameButtonName, String sortByDateButtonName, String gameNameFieldName) {
    this.SAVETIME_ORDER_text = null;
    this.SAVETIME_ORDER_QR_text = null;
    ...
    this.warning_replace = new PoPUpMenu(_menu, this.WARNING_XML, "Tablegroup - CONFIRM REPLACE", "CONFIRM REPLACE");
    switch (this.menu_load_type) {
      case 0:
  ...
  class NotCompatible implements IPoPUpMenuListener {
    int menu_load_type;
